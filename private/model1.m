...
[x_interval, t_interval] = form_intervals(1000, -40.0, 40.0, 0.0, 300);
% Step 4
...
% Step 6
c_integral = [-0.65 1.25];
pieces = 10000;
t_matr = create_functional_t(c_integral, khi_plus, khi_minus, pieces);
t_matr2 = create_functional_t(c_integral, khi_plus, khi_minus, pieces);
int1 = -integral(@(y)power(y .* khi_plus(1 ./ y, 0.0), -2.0), ...
    pieces, power(pieces, 5.0)) + c_integral(1);
int2 = -integral(@(y)power(y .* khi_minus(1 ./ y, 0.0), -2.0), ...
    pieces, power(pieces, 5.0)) + c_integral(2);
t_matr2{1, 1} = @(x)(t_matr2{1, 2}(x) * int1);
t_matr2{2, 1} = @(x)(t_matr2{2, 2}(x) * int2);
% Step 7
F_func = @(t, x)determinate_T(t_matr2, t, x);
...
init_point = linspace(x_interval(1), x_interval(end), ...
    wave_count) + 1e-12;
